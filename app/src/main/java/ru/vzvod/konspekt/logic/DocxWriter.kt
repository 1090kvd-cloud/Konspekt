...
object DocxWriter {
    ...
    /** Абзац с рисунком: размеры берём из самого файла, ширину ограничиваем полосой набора. */
    private fun image(name: String): String {
        ...
        val maxW = 5_400_000L          // 15 см в EMU
        val maxH = 7_200_000L          // 20 см, чтобы рисунок не занял всю страницу
        var cx = maxW
        var cy = cx * h / w
        if (cy > maxH) {
            cy = maxH
    ...
    private fun document(plan: LessonPlan, s: Settings): String {
        ...
        body.append(p("«___» __________ 20___ г.", align = "right"))
        body.append(p(""))
...
<!--images--><Override PartName="/word/document.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml"/>
</Types>"""
