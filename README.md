...
Разбираются: `.docx`, `.txt`, `.rtf`, `.html`, `.md`, `.csv`, а также `.doc`, сохранённый как HTML. Не разбираются: PDF, фотографии и старый двоичный `.doc` — приложение честно говорит об этом и предлагает пересохранить файл в `.docx`.
...
Разбор идёт по меткам формы. Чего в файле нет — остаётся пустым; приложение ничего не додумывает и после загрузки пишет, что именно распозналось в каждом файле.
